...
class ExtractService:
    ...
    def get_commits_data(self, commits, gh_commit: GHCommit) -> list[dict]:
        total_commits: list[dict] = []
    ...
    def gh_get_new_commits(self, project_name: str, last_commit_data: dict):
        ...
        commits = gh_commit.get_commits_between_dates(since=since, until=TEST_DATE)  # type: ignore
        new_info = True if len(list(commits)) > 0 else False
        if not new_info:
            return None
    ...
    def db_update_last_commit(self, data: list):
