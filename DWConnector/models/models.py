from sqlalchemy import Column, Integer, String, TIMESTAMP, ForeignKey, DateTime, Boolean, DECIMAL, Index, UniqueConstraint
...
class Project(Base):
    ...
    forked_from = Column(Integer, ForeignKey("projects.id"))
    ...
    pull_requests = relationship('PullRequest', foreign_keys='PullRequest.base_repo_id', backref='project')
...
class PullRequest(Base):
    ...
    __table_args__ = (
        UniqueConstraint('pullreq_id', 'base_repo_id', name='_pullreq_base_uc'),
    )
...
class Commit(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_committer_id', committer_id),
        Index('idx_project_id', project_id),
        UniqueConstraint('sha', name='uq_commits_sha')
    )
