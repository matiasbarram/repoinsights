...
from services.traspaso_service.traspaso.common_clases import (
    NotFoundHandler,
)
...
class ConsolidatedClient:
    def __init__(self, uuid: str, db: DatabaseHandler) -> None:
        ...
        self.not_found_handler = NotFoundHandler(self.db)
        self.cache = Cache(self.not_found_handler, self.db)
    ...
    def add_users(self, users: List[User]):
        for user in users:
            ...
            if existing_user is None:
                ...
                self.cache.user_id_map[old_id] = new_user.id
            else:
                self.cache.user_id_map[user.id] = existing_user.id
    ...
    def add_main_project(self, projects: List[Project]):
        ...
        exist_in_consolidada = (
            ...
            .filter_by(
                owner_id=self.cache.get_from_map(
                    self.cache.user_id_map, project.owner_id
                ),
                name=project.name,
            )
            .first()
        ...
        if exist_in_consolidada:
            self.cache.project_id_map[project.id] = exist_in_consolidada.id
            return exist_in_consolidada.id

        consolidada_main_project = Project(
            url=project.url,
            owner_id=self.cache.get_from_map(self.cache.user_id_map, project.owner_id),
            name=project.name,
        ...
        self.db.session_consolidada.commit()
        self.cache.project_id_map[project.id] = consolidada_main_project.id
    ...
    def add_projects(self, projects: List[Project]):
        ...
        for project in projects:
            ...
            if existing_project is None:
                ...
                self.db.session_consolidada.commit()
                self.cache.project_id_map[old_id] = new_project.id
            else:
                self.cache.project_id_map[project.id] = existing_project.id

    def add_commits(self, commits: List[Commit]):
        for commit in commits:
            project_id = self.cache.get_from_map(
                self.cache.project_id_map, commit.project_id
            )
            author_id = self.cache.get_from_map(
                self.cache.user_id_map, commit.author_id
            )
            committer_id = self.cache.get_from_map(
                self.cache.user_id_map, commit.committer_id
            )
            existing_commit = (
            ...
            if existing_commit is None:
                ...
                self.db.session_consolidada.commit()
                self.cache.commit_id_map[commit.id] = new_commit.id
            else:
                self.cache.commit_id_map[commit.id] = existing_commit.id
    ...
    def add_commit_comments(self, commit_comments: List[CommitComment]):
        for commit_comment in commit_comments:
            ...
            commit_id = self.cache.get_from_map(
                self.cache.commit_id_map, commit_comment.commit_id
            )
            ...
            existing_commit_comment = (
                ...
                .filter_by(
                    commit_id=commit_id,
                    user_id=self.cache.get_from_map(
                        self.cache.user_id_map, commit_comment.user_id
                    ),
                )
                .first()
            )
            ...
            if existing_commit_comment is None:
                ...
                self.db.session_consolidada.commit()
                self.cache.commit_comment_id_map[
                    commit_comment.id
                ] = new_commit_comment.id
    ...
    def add_pull_requests(self, pull_requests: List[PullRequest]):
        for pull_request in pull_requests:
            ...
            head_repo_id = self.cache.get_from_map(
                self.cache.project_id_map, pull_request.head_repo_id
            )
            base_repo_id = self.cache.get_from_map(
                self.cache.project_id_map, pull_request.base_repo_id
            )
            head_commit_id = self.cache.get_from_map(
                self.cache.commit_id_map, pull_request.head_commit_id
            )
            base_commit_id = self.cache.get_from_map(
                self.cache.commit_id_map, pull_request.base_commit_id
            )
            user_id = self.cache.get_from_map(
                self.cache.user_id_map, pull_request.user_id
            )
            ...
            if existing_pull_request is None:
                ...
                self.db.session_consolidada.commit()
                self.cache.pull_request_id_map[pull_request.id] = new_pull_request.id
            else:
                self.cache.pull_request_id_map[
                    pull_request.id
                ] = existing_pull_request.id

    def add_issues(self, issues: List[Issue]):
        for issue in issues:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            repo_id = self.cache.get_from_map(self.cache.project_id_map, issue.repo_id)
            reporter_id = self.cache.get_from_map(
                self.cache.user_id_map, issue.reporter_id
            )
            assignee_id = self.cache.get_from_map(
                self.cache.user_id_map, issue.assignee_id
            )
            pull_request_id = self.cache.get_from_map(
                self.cache.pull_request_id_map, issue.pull_request_id
            )
            ...
            if existing_issue is None:
                ...
                self.db.session_consolidada.commit()
                self.cache.issue_id_map[issue.id] = new_issue.id
            else:
                self.cache.issue_id_map[issue.id] = existing_issue.id
    ...
    def add_pull_request_comments(
        ...
        for pull_request_comment in pull_request_comments:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            pull_request_id = self.cache.get_from_map(
                self.cache.pull_request_id_map, pull_request_comment.pull_request_id
            )

            user_id = self.cache.get_from_map(
                self.cache.user_id_map, pull_request_comment.user_id
            )
            commit_id = self.cache.get_from_map(
                self.cache.commit_id_map, pull_request_comment.commit_id
            )
    ...
    def add_pull_request_history(self, pull_request_history: List[PullRequestHistory]):
        for pull_request_history in pull_request_history:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            pull_request_id = self.cache.get_from_map(
                self.cache.pull_request_id_map, pull_request_history.pull_request_id
            )
            actor_id = self.cache.get_from_map(
                self.cache.user_id_map, pull_request_history.actor_id
            )
            ...
            if existing_pull_request_history is None:
                ...
                self.db.session_consolidada.commit()
                self.cache.pull_request_history_id_map[
    ...
    def add_issue_comments(self, issue_comments: List[IssueComment]):
        for issue_comment in issue_comments:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            issue_id = self.cache.get_from_map(
                self.cache.issue_id_map, issue_comment.issue_id
            )
            user_id = self.cache.get_from_map(
                self.cache.user_id_map, issue_comment.user_id
            )
    ...
    def add_issue_events(self, issue_events: List[IssueEvent]):
        for issue_event in issue_events:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            issue_id = self.cache.get_from_map(
                self.cache.issue_id_map, issue_event.issue_id
            )
            actor_id = self.cache.get_from_map(
                self.cache.user_id_map, issue_event.actor_id
            )
    ...
    def add_labels(self, labels: List[RepoLabel]):
        for label in labels:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            repo_id = self.cache.get_from_map(self.cache.project_id_map, label.repo_id)
            ...
            if existing_label is None:
                ...
                self.db.session_consolidada.commit()
                self.cache.label_id_map[label.id] = new_label.id
            else:
                self.cache.label_id_map[label.id] = existing_label.id

    def add_issue_labels(self, issue_labels: List[IssueLabel]):
        for issue_label in issue_labels:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            label_id = self.cache.get_from_map(
                self.cache.label_id_map, issue_label.label_id
            )
            issue_id = self.cache.get_from_map(
                self.cache.issue_id_map, issue_label.issue_id
            )
    ...
    def add_milestones(self, milestones: List[RepoMilestone]):
        for milestone in milestones:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            repo_id = self.cache.get_from_map(
                self.cache.project_id_map, milestone.repo_id
            )
    ...
    def add_watchers(self, watchers: List[Watcher]):
        for watcher in watchers:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            repo_id = self.cache.get_from_map(
                self.cache.project_id_map, watcher.repo_id
            )
            user_id = self.cache.get_from_map(self.cache.user_id_map, watcher.user_id)
    ...
    def add_commit_parents(self, commit_parents: List[CommitParent]):
        for commit_parent in commit_parents:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            commit_id = self.cache.get_from_map(
                self.cache.commit_id_map, commit_parent.commit_id
            )
            parent_id = self.cache.get_from_map(
                self.cache.commit_id_map, commit_parent.parent_id
            )
    ...
    def add_followers(self, followers: List[Follower]):
        for follower in followers:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            user_id = self.cache.get_from_map(self.cache.user_id_map, follower.user_id)
            follower_id = self.cache.get_from_map(
                self.cache.user_id_map, follower.follower_id
            )
    ...
    def add_project_members(self, project_members: List[ProjectMember]):
        for project_member in project_members:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            repo_id = self.cache.get_from_map(
                self.cache.project_id_map, project_member.repo_id
            )
            user_id = self.cache.get_from_map(
                self.cache.user_id_map, project_member.user_id
            )
    ...
    def add_extractions(self, extractions: List[Extraction]):
        for extraction in extractions:
            # Obtén el proyecto correspondiente en la base de datos consolidada
            project_id = self.cache.get_from_map(
                self.cache.project_id_map, extraction.project_id
            )
